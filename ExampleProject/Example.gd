...
func _ready() -> void:
	...
	icons.load_from_path("res://ExampleProject/Icons.cfg")
	icons.load_from_path("res://ExampleProject/Icons.json")
	
	# Create icons, reading their color and scale from the database.
	for entry in icons.get_array():
		var icon := Sprite2D.new()
		icon.name = entry.name
		icon.texture = preload("res://ExampleProject/icon.png")
	...
	var weapons := TextDatabase.load_database("res://ExampleProject/CustomDatabase.gd", "res://ExampleProject/Weapons.cfg")
