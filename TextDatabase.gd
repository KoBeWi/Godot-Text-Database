...
func add_default_property(property: String, default, typed := true) -> void:
...
func is_property_valid(entry: Dictionary, property: String, value = null) -> bool:
	...
	if not valid:
		for prop: Array in __mandatory_properties:
			if prop[0] == property:
...
func load_from_path(path: String) -> void:
	...
	for entry: Dictionary in data:
		for property: Array in __default_values:
...
func __property_exists(property: String) -> bool:
	for p: Array in __mandatory_properties:
		if p[0] == property:
			return true
